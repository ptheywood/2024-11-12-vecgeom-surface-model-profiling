...
# Profiling the 4 Surface modelling approaches {.divider .teal visibility="uncounted" data-hide-slide-number='true'}
...
- Higher occupancy does not guarantee higher performance
  - **But** low occupancy always reduces latency hiding
    - I.e. *long scoreboard stalls*
  - And if too low cannot exploit full GPU
...
## Set maximum registers per thread to use
...
- Force NVCC to limit registers per thread
  - For all kernels via `--maxrregcount N`
    - Per kernel:
      ...
      - `__launch_bounds__` for older CUDAs (less intuitive)
:::
...
::: {.column .somewhat-smaller width="60%"}
...
| Strategy               | Reference Time(s) | Time | Selected Blocksizes |
|------------------------|----------------|------|---------------------|
