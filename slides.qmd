...
![GPU-only timeline (TBHGCal, `524228` rays, `use_tb_gun 1`, V100)](img/nsys-overveiws/nvtx-timeline-gpu-only-tbhgcal-524228-tbgun-1.png){fig-alt="Annotated NSight Systems timeline for CPU & GPU testRaytracing for TBHGCal with 65536 rays and -use_TB_gun 0" width="100%" height="100%"}
...
![Initial `testRaytracing` benchmarking`](img/initial-testRaytracing-10M-gun-0.png){fig-alt="Initial testRaytracing benchmarking" width="100%" height="100%"}

- 3 geometries
- 10 million rays (unrealistic rays for TBGHCal)
- CPU & GPU

# Surface Model Kernels {.divider .teal}
...
![Full GH100. Copyright © 2022 NVIDIA Corporation ([source](https://developer.nvidia.com/blog/nvidia-hopper-architecture-in-depth/))](img/nvidia/Full-H100-GPU-with-144-SMs-1024x457.png)
...
![GH100 Streaming Multiprocessor. Copyright © 2022 NVIDIA Corporation ([source](https://developer.nvidia.com/blog/nvidia-hopper-architecture-in-depth/))](img/nvidia/H100-Streaming-Multiprocessor-SM-625x869.png)

- L2 Cache & Global memory are outside the SM
  - i.e. higher latency

## Occupancy @todo split up & rename

<!-- @todo - thread block diagram from phd? -->

- Threads are submit in a hierarchy, as a **grid** of **blocks** of **threads**
- Blocks are issued to an SM, and they become **resident**
  - Remain on the SM until they terminate
  - Limit on number of resident threads due to hardware constraints

- A **warp** is the group of `32` threads which execute instructions in lock-step 
  - **Active threads** are participating in the current instruction within the warp
  - **Disabled threads** are not participating
...
- **Occupancy** - Ratio of active warps on an SM to the maximum it could support
  ...
  - **Achieved Occupancy** - the actual ratio at execution time
    - lower than theoretical due to branching, stalls, workload imbalance

- Compiler selects the number of **registers per thread** at compile time
  - Hardware maximum of `255` registers per thread in recent HPC GPUs 
...
- @todo - ncu occupancy is low (12.5%)
- @todo - ncu screenshots showing registers per thread / occupancy for one or both of the above kernels
