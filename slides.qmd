...
+ CPU + GP
+ Solid modeling is not ideal for GPU
+ Surface modeling approach for GPUs
...
+ Loads Geometry, generates random rays, tests on CPU and GPU.
  ...
  + Validation (optional)
  + Without/with BVH
  + BVH with multiple kernel launches & split kernels
...
+ Rather than using a single kernel launch for the full ray propagation phase, split into multiple launches
+ Less registers per thread may be required


## `ComputeNextStepAndNextSurfaces` + `LocateSurf` 

**(or `RelocateToNextVolumes` and I didn't profile it...) @todo**
